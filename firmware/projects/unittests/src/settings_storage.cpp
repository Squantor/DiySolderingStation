...
/**
 * @brief Settings storage buffer checksum
 * @return std::uint16_t
 */
std::uint16_t settings_buffer_checksum() {
  uint16_t sum = settings_buffer[0] + settings_buffer[1];
  for (std::size_t i = 4; i < settings_buffer.size(); i++) {
    sum += settings_buffer[i];
  }
  return sum;
}

MINUNIT_SETUP(settings_storage_setup) {
  settings_buffer.fill(0);
...
MINUNIT_ADD(settings_storage_init_empty, settings_storage_setup, settings_storage_teardown) {
  ...
  Test_storage test_storage_defaults;
  std::memset(&test_storage_defaults, 0, sizeof(test_storage_defaults));
  test_storage_defaults.value_one = 4;
  test_storage_defaults.value_two = 5;
  test_storage_defaults.value_three = 6;
